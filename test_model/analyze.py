...
tol = 1e-3                      # Tolerance for rank calculation

# Dimensional matrix
# Column order = {P_0,P_e,A_e,U_0,U_e,a_0}
D = np.array(dim_mat)
N = ut.null(D)
...
B_1 = P_N.dot(W_1);  r_1 = np.linalg.matrix_rank(B_1,tol=tol)  
B_2 = P_Nc.dot(W_1); r_2 = np.linalg.matrix_rank(B_2,tol=tol)  
...
print("Rank(dim_as) = {}".format(np.linalg.matrix_rank(dim_as,tol=tol)))
